# frozen_string_literal: true

# Be sure to restart your server when you modify this file.

# Define an application-wide content security policy.
# See the Securing Rails Applications Guide for more information:
# https://guides.rubyonrails.org/security.html#content-security-policy-header

Rails.application.configure do
  config.content_security_policy do |policy|
    policy.default_src :self
    policy.script_src  :self, "rv.e6ai.net", "https://www.google.com/recaptcha/", "https://www.gstatic.com/recaptcha/", "https://www.recaptcha.net/", "https://assets.freespeechcoalition.com", "https://plausible.dragonfru.it/"
    policy.script_src(*policy.script_src, :unsafe_eval) if Rails.env.development?

    policy.style_src :self, :unsafe_inline
    policy.style_src(*policy.style_src, :unsafe_inline) if Rails.env.development?

    policy.connect_src :self, "rv.e6ai.net", "plausible.dragonfru.it", "static1.e6ai.net", "api.freespeechcoalition.com"
    policy.connect_src(*policy.connect_src, "ws://localhost:3036", "http://localhost:3036") if Rails.env.development?

    policy.object_src  :self, "static1.e6ai.net"
    policy.media_src   :self, "static1.e6ai.net"
    policy.frame_ancestors :none
    policy.frame_src   "https://www.google.com/recaptcha/", "https://www.recaptcha.net/"
    policy.font_src    :self
    policy.img_src     :self, :data, "static1.e6ai.net", "rv.e6ai.net"
    policy.child_src   :none
    policy.form_action :self
    # Specify URI for violation reports
    # policy.report_uri "/csp-violation-report-endpoint"
  end

  # Generate session nonces for permitted importmap and inline scripts
  config.content_security_policy_nonce_generator = ->(_request) { SecureRandom.base64(16) }
  config.content_security_policy_nonce_directives = %w[script-src]

  # Report violations without enforcing the policy.
  config.content_security_policy_report_only = false
end
